 ...
 * Sincronizado com rpc_get_open_sale v4.0.
 */
...
import { Plus, Search, ArrowRight, ClipboardList, MapPin, User, RefreshCw } from 'lucide-react';
...
export default function ComandasPage() {
  const { refreshStatus, comandas, storeStatus } = useAuth();
          ...
          >
            ...
            <CardHeader className="bg-muted/20 border-b py-4">
              ...
              <div className="flex flex-col gap-1 mt-2">
                <div className="flex items-center gap-1.5 text-[10px] font-black uppercase text-primary">
                  <MapPin className="h-3 w-3" /> {comanda.table_number === 0 ? 'Entrega Rápida' : `Local ${comanda.table_number}`}
                </div>
            ...
            <CardFooter className="pt-0 pb-4 flex justify-end">
              <span className="text-[10px] font-black uppercase text-primary flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                Gerenciar Pedidos <ArrowRight className="h-3 w-3" />
              </span>
            </CardFooter>
          </Card>
        ))}
